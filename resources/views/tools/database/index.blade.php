...
@section('page_header')
    <h1 class="page-title">
        <i class="softadmin-data"></i> Database
        <a href="{{ route('softadmin.database.create') }}" class="btn btn-success"><i class="softadmin-plus"></i>
            Create New Table</a>
    </h1>
@stop
    ...
    <div class="page-content container-fluid">
        <div class="row">
            <div class="col-md-12">
                ...
                <?php $dataTypes = SBD\Softadmin\Models\DataType::all(); ?>
                <?php $dataTypeNames = []; ?>
                ...
                <?php $arr = SBD\Softadmin\Facades\DBSchema::tables(); ?>
                @foreach($arr as $a)
	            <?php $table = current($a); ?>
		    <?php if (in_array($table, config('softadmin.database.tables.hidden', []))) continue; ?>
                	<?php $active = in_array($table, $dataTypeNames); 
                        if($active){
                            $activeDataType = SBD\Softadmin\Models\DataType::where('name', '=', $table)->first();
                        }
                    ?>
                        ...
                        <tr>
                            <td>
                                <p class="name">
                                    @if($active)
                                        <a href="{{ route('softadmin.database.show', $table) }}"
                                           data-name="{{ $table }}" class="desctable">{{ $table }}</a> <i
                                                class="softadmin-bread"
                            ...
                            <td class="actions">
                                <a class="btn-danger btn-sm pull-right delete_table @if($active) remove-bread-warning @endif"
                                   data-table="{{ $table }}" style="display:inline; cursor:pointer;"><i
                                            class="softadmin-trash"></i> Delete</a>
                                <a class="btn-sm btn-primary pull-right" style="display:inline; margin-right:10px;"
                                   href="{{ route('softadmin.database.edit', $table) }}"><i
                                            class="softadmin-edit"></i> Edit</a>
                                <a class="btn-sm btn-warning pull-right desctable"
                                   style="display:inline; margin-right:10px;"
                                   href="{{ route('softadmin.database.show', $table) }}" data-name="{{ $table }}"><i
                                            class="softadmin-eye"></i> View</a>
                            </td>
                        </tr>
                    @endforeach
                </table>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="softadmin-trash"></i> Are you sure you want to delete the BREAD for
                        the <span id="delete_builder_name"></span> table?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('softadmin.database.delete_bread', ['id' => null]) }}" id="delete_builder_form" method="POST">
                        {{ method_field('DELETE') }}
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="softadmin-trash"></i> Are you sure you want to delete the <span
                                id="delete_table_name"></span> table?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('softadmin.database.destroy', ['database' => '__database']) }}" id="delete_table_form" method="POST">
                        {{ method_field('DELETE') }}
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="softadmin-data"></i> @{{ table.name }}</h4>
                </div>
