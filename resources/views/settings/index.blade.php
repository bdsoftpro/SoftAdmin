...
@section('css')
    <style>
        .panel-actions .softadmin-trash {
            cursor: pointer;
        }

        .panel-actions .softadmin-trash:hover {
            color: #e94542;
        }
...
@section('page_header')
    <h1 class="page-title">
        <i class="softadmin-settings"></i> Settings
    </h1>
@stop
...
@section('content')
    ...
    <div class="container-fluid">
        <div class="alert alert-info">
            ...
            <p>You can get the value of each setting anywhere on your site by calling <code>Softadmin::setting('key')</code></p>
        </div>
    </div>

    <div class="page-content container-fluid">
        <form action="{{ route('softadmin.settings.update') }}" method="POST" enctype="multipart/form-data">
            {{ method_field("PUT") }}
            {{ csrf_field() }}
            <div class="panel">
                @foreach($settings as $setting)
                    <div class="panel-heading">
                        <h3 class="panel-title">
                            {{ $setting->display_name }}<code>Softadmin::setting('{{ $setting->key }}')</code>
                        </h3>
                        <div class="panel-actions">
                            <a href="{{ route('softadmin.settings.move_up', $setting->id) }}">
                                <i class="sort-icons softadmin-sort-asc"></i>
                            </a>
                            <a href="{{ route('softadmin.settings.move_down', $setting->id) }}">
                                <i class="sort-icons softadmin-sort-desc"></i>
                            </a>
                            <i class="softadmin-trash"
                               data-id="{{ $setting->id }}"
                    ...
                    <div class="panel-body">
                        ...
                        @elseif($setting->type == "image" || $setting->type == "file")
                            @if(isset( $setting->value ) && !empty( $setting->value ) && Storage::exists(config('softadmin.storage.subfolder') . $setting->value))
                                <div class="img_settings_container">
                                    <a href="{{ route('softadmin.settings.delete_value', $setting->id) }}" class="softadmin-x"></a>
                                    <img src="{{ Storage::url(config('softadmin.storage.subfolder') . $setting->value) }}" style="width:200px; height:auto; padding:2px; border:1px solid #ddd; margin-bottom:10px;">
                                </div>
        ...
        <div class="panel" style="margin-top:10px;">
            <div class="panel-heading new-setting">
                <hr>
                <h3 class="panel-title"><i class="softadmin-plus"></i> New Setting</h3>
            </div>
            <div class="panel-body">
                <form action="{{ route('softadmin.settings.store') }}" method="POST">
                    {{ csrf_field() }}
                    <div class="col-md-4">
                    ...
                    <div class="col-md-12">
                        <a id="toggle_options"><i class="softadmin-double-down"></i> OPTIONS</a>
                    ...
                    <script>
                        $('document').ready(function () {
                            $('#toggle_options').click(function () {
                                ...
                                if ($('#toggle_options .softadmin-double-down').length) {
                                    $('#toggle_options .softadmin-double-down').removeClass('softadmin-double-down').addClass('softadmin-double-up');
                                } else {
                                    $('#toggle_options .softadmin-double-up').removeClass('softadmin-double-up').addClass('softadmin-double-down');
                                }
                            });
                        });
                    </script>
                    ...
                    <button type="submit" class="btn btn-primary pull-right new-setting-btn">
                        <i class="softadmin-plus"></i> Add New Setting
                    </button>
                    <div style="clear:both"></div>
                </form>
            </div>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">
                        <i class="softadmin-trash"></i> Are you sure you want to delete the <span id="delete_setting_title"></span> Setting?
                    </h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('softadmin.settings.delete', ['id' => '__id']) }}" id="delete_form" method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
    ...
    <script>
        $('document').ready(function () {
            $('.softadmin-trash').click(function () {
...
@section('javascript')
    ...
    <form id="my_form" action="{{ route('softadmin.upload') }}" target="form_target" method="POST" enctype="multipart/form-data" style="width:0;height:0;overflow:hidden">
        {{ csrf_field() }}
