...
@section('page_header')
    <h1 class="page-title">
        <i class="softadmin-list"></i>Menu Builder ({{ $menu->name }})
        <div class="btn btn-success add_item"><i class="softadmin-plus"></i> New Menu Item</div>
    </h1>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="softadmin-trash"></i> Are you sure you want to delete this menu
                        item?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('softadmin.menus.item.destroy', ['menu' => $menu->id, 'id' => '__id']) }}" id="delete_form"
                          method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="softadmin-plus"></i> Create a New Menu Item</h4>
                </div>
                <form action="{{ route('softadmin.menus.item.add', ['menu' => $menu->id]) }}" id="delete_form" method="POST">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="softadmin-edit"></i> Edit Menu Item</h4>
                </div>
                <form action="{{ route('softadmin.menus.item.update', ['menu' => $menu->id]) }}" id="edit_form" method="POST">
                    {{ method_field("PUT") }}
                    {{ csrf_field() }}
                    <div class="modal-body">
                        <label for="name">Title of the Menu Item</label>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.dd').on('change', function (e) {
                $.post('{{ route('softadmin.menus.order',['menu' => $menu->id]) }}', {
