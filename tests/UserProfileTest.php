...
class UserProfileTest extends TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->editPageForTheCurrentUser = route('softadmin.users.edit', ['user' => $this->user->id]);

        $this->listOfUsers = route('softadmin.users.index');
    }

    public function testCanSeeTheUserInfoOnHisProfilePage()
    {
        $this->visit(route('softadmin.profile'))
    ...
    public function testCanEditUserPassword()
    {
        $this->visit(route('softadmin.profile'))
             ...
             ->see('Edit User')
             ->seePageIs($this->editPageForTheCurrentUser)
             ->type('softadmin-rocks', 'password')
             ->press('Submit')
             ->seePageIs($this->listOfUsers);

        $updatedPassword = DB::table('users')->where('id', 1)->first()->password;
        $this->assertTrue(Hash::check('softadmin-rocks', $updatedPassword));
    }
