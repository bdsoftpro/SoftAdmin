...
class RolesTest extends TestCase
{
    ...
    public function testRoles()
    {
        ...
        // Adding a New Role
        $this->visit(route('softadmin.roles.index'))->click('Add New')->seePageIs(route('softadmin.roles.create'));
        $this->type('superadmin', 'name');
        $this->type('Super Admin', 'display_name');
        $this->press('Submit');
        $this->seePageIs(route('softadmin.roles.index'));
        $this->seeInDatabase('roles', ['name' => 'superadmin']);

        // Editing a Role
        $this->visit(route('softadmin.roles.edit', 2));
        ...
        // Deleting a Role
        $response = $this->call('DELETE', route('softadmin.roles.destroy', $superadmin_role->id), ['_token' => csrf_token()]);
